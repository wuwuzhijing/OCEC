#!/bin/bash
# ============================================================
# MRL Eye Dataset 独立训练 v4.5 (预训练 backbone)
# ============================================================
#
# v4.5 改进 (vs v2):
...
#   - 参数量 ~1.5M（v2 自研 ~0.3M，v4.5 容量大且特征质量高）
#
# 可选 backbone (torchvision): mobilenet_v3_small | efficientnet_b0 | resnet18 | resnet34
#              (timm): repvgg_a0 | repvgg_a1 | repvgg_a2 | mnasnet | repvgg_b1 | repvgg_b2
#
...
OUTPUT_DIR="runs/ocec_mrl_v4.5_mnasnet"
...
LOG_FILE="logs/train/mrl/train_mrl_v4.5_mnasnet_$(date +%Y%m%d_%H%M%S).log"
mkdir -p logs/train/mrl

nohup bash -c "source \$(conda info --base)/etc/profile.d/conda.sh && conda activate tf_310 && exec -a mrl_v4.5 python - << 'EOF'
import os, shlex
...
if [ -n "$RESUME_ARG" ]; then
    echo "  MRL v4.5 续训已启动 (resume)"
else
    echo "  MRL v4.5 独立训练已启动 (fresh)"
fi
echo "============================================"
echo "  v4.5 改进: pretrained ${BACKBONE} + 64x64"
