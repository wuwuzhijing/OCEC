...
def main():
    ...
    if images_dir is not None:
        dataset_label = Path(images_dir).name
    else:
        if is_parsable_to_int(video):
            dataset_label = f'camera_{video}'
        else:
            dataset_label = Path(video).stem
    dataset_label = sanitize_label(dataset_label)
    eye_stats = EyeAnalysisStats()
    ...
    while True:
        ...
        eye_stats.register_frame(eye_boxes)
    ...
    if eye_stats.total_frames > 0:
        print(Color.CYAN(f'[Eye Analysis] {dataset_label}'))
        print(f'  Total frames processed: {eye_stats.total_frames}')
        print(f'  Frames with Eye detections: {eye_stats.frames_with_eye}')
        print(f'  Frames without Eye detections: {eye_stats.frames_without_eye}')
        print(f'  Frames with ≥3 Eye detections: {eye_stats.frames_over_detect}')
        ...
        if eye_stats.has_measurements:
            ...
            hist_stats = plot_eye_histograms(
                ...
                output_path=combined_hist_path,
            )

            if hist_stats:
                print(f'  Histogram PNG: {combined_hist_path}')
                width_stats = hist_stats.get('width') or {}
                height_stats = hist_stats.get('height') or {}
                if width_stats:
                    print(f'    Width  -> mean={width_stats["mean"]:.2f}, median={width_stats["median"]:.2f}')
                else:
                    print('    Width  -> No width data available')
                if height_stats:
                    print(f'    Height -> mean={height_stats["mean"]:.2f}, median={height_stats["median"]:.2f}')
                else:
                    print('    Height -> No height data available')
        else:
            print('  No valid Eye bounding boxes detected; histograms were not generated.')
    else:
        print(Color.YELLOW(f'[Eye Analysis] No frames processed for {dataset_label}.'))
