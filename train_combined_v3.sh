...
#
# 可选 backbone: mobilenet_v3_small | efficientnet_b0 | resnet18
#
# ⚠️  严格区分:
#   - MRL v3:  train_mrl_v3.sh      → runs/ocec_mrl_v3/      port 6006
#   - 联合 v3:  train_combined_v3.sh → runs/ocec_combined_v3/ port 6007
# ============================================================
...
OUTPUT_DIR="runs/ocec_combined_v3"
...
LOG_FILE="logs/train/combined/train_combined_v3_$(date +%Y%m%d_%H%M%S).log"
mkdir -p logs/train/combined

nohup bash -c "source \$(conda info --base)/etc/profile.d/conda.sh && conda activate tf_310 && exec -a combined_v3 python - << 'EOF'
import os, shlex
...
if [ -n "$RESUME_ARG" ]; then
    echo "  联合训练 v3 续训已启动 (resume)"
else
    echo "  联合训练 v3 已启动 (fresh)"
fi
echo "============================================"
echo "  v3 改进: pretrained ${BACKBONE} + 64x64"
