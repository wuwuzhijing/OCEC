...
# 2.2.2：修复margin在tensorboard中为直线的问题
# ========== 配置区域 ==========
# 模型A的最佳checkpoint路径
PRETRAIN_MODEL="runs/ocec_hq_finetune_progressive_v2.2.2/v3/ocec_epoch_0047.pt"

# 数据集B的路径
DATA_ROOT_B="/ssddisk/guochuang/ocec/parquet_hq_v2"

# 微调输出目录
OUTPUT_DIR="runs/ocec_hq_finetune_progressive_v3.0"
...
export OCEC_ARGS="train \
    ...
    --use_amp \
    --pseudo_update_start_epoch 1000"

    # --freeze_backbone \

# 激活 conda 环境并运行训练
LOG_FILE="logs/train/hq_data/train_finetune_progressive_$(date +%Y%m%d_%H%M%S).log"
nohup bash -c "source $(conda info --base)/etc/profile.d/conda.sh && conda activate deploy && exec -a fprog3 python - << 'EOF'
import os, shlex
