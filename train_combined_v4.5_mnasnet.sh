...
OUTPUT_DIR="runs/ocec_combined_v4.5_mnasnet"
...
LOG_FILE="logs/train/combined/train_combined_v4.5_mnasnet_$(date +%Y%m%d_%H%M%S).log"
mkdir -p logs/train/combined

nohup bash -c "source \$(conda info --base)/etc/profile.d/conda.sh && conda activate tf_310 && exec -a combined_v4.5 python - << 'EOF'
import os, shlex
...
if [ -n "$RESUME_ARG" ]; then
    echo "  联合训练 v4.5 续训已启动 (resume)"
else
    echo "  联合训练 v4.5 已启动 (fresh)"
fi
echo "============================================"
