...
# ========== 配置区域 ==========
# 模型A的最佳checkpoint路径
PRETRAIN_MODEL="runs/ocec_6Whq+24WPublic/v8/ocec_best_epoch0208_f1_0.9845.pt"

# 数据集B的路径
DATA_ROOT_B="/ssddisk/guochuang/ocec/parquet_hq"

# 微调输出目录
OUTPUT_DIR="runs/ocec_hq_finetune_progressive"

# 第一阶段配置（冻结backbone，只训练head）
STAGE1_EPOCHS=10          # 第一阶段训练轮数
STAGE1_LR=1e-4            # 第一阶段学习率（head可以稍大）

# 第二阶段配置（解冻部分backbone）
STAGE2_EPOCHS=100         # 第二阶段训练轮数
STAGE2_LR=5e-6            # 第二阶段学习率（backbone需要很小）
UNFREEZE_EPOCH=2         # 在哪个epoch解冻backbone（STAGE1_EPOCHS + 1）
