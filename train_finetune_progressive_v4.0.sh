...
# ========== 配置区域 ==========
# 模型A的最佳checkpoint路径
PRETRAIN_MODEL="runs/ocec_6Whq+24WPublic/v8/ocec_best_epoch0126_f1_0.9845.pt"

# 数据集B的路径
DATA_ROOT_B="/ssddisk/guochuang/ocec/parquet_hq_v2"

# 微调输出目录
OUTPUT_DIR="runs/ocec_hq_finetune_progressive_v4.0"

# 第一阶段配置（冻结backbone，只训练head）
STAGE1_EPOCHS=20           # 第一阶段训练轮数
STAGE1_LR=2e-4             # 第一阶段学习率
...
nohup bash -c "source $(conda info --base)/etc/profile.d/conda.sh && conda activate deploy && exec -a fprog4 python - << 'EOF'
import os, shlex
