#!/bin/bash
# ============================================================
# MRL Eye Dataset 独立训练 v3 (预训练 backbone)
# ============================================================
#
# v3 改进 (vs v2):
...
#   - 参数量 ~1.5M（v2 自研 ~0.3M，v3 容量大且特征质量高）
#
...
LOG_FILE="logs/train/mrl/train_mrl_v3_$(date +%Y%m%d_%H%M%S).log"
mkdir -p logs/train/mrl

nohup bash -c "source \$(conda info --base)/etc/profile.d/conda.sh && conda activate tf_310 && exec -a mrl_v3 python - << 'EOF'
import os, shlex
...
if [ -n "$RESUME_ARG" ]; then
    echo "  MRL v3 续训已启动 (resume)"
else
    echo "  MRL v3 独立训练已启动 (fresh)"
fi
echo "============================================"
