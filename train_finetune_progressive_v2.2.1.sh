...
# 2.2.1：修复margin在tensorboard中为直线的问题
# ========== 配置区域 ==========
# 模型A的最佳checkpoint路径
PRETRAIN_MODEL="runs/ocec_hq_finetune_progressive_v2.2.1/v10/ocec_best_epoch0003_f1_0.7655.pt"

# 数据集B的路径
DATA_ROOT_B="/ssddisk/guochuang/ocec/parquet_hq_v2"

# 微调输出目录
OUTPUT_DIR="runs/ocec_hq_finetune_progressive_v2.2.2"

# 第一阶段配置（冻结backbone，只训练head）
STAGE1_EPOCHS=2           # 第一阶段训练轮数
STAGE1_LR=5e-6            # 第一阶段学习率

# 第二阶段配置（解冻部分backbone）
STAGE2_EPOCHS=1000        # 第二阶段训练轮数
STAGE2_LR=2e-6            # 第二阶段学习率
UNFREEZE_EPOCH=2         # 在哪个epoch解冻backbone（STAGE1_EPOCHS + 1）,v2.1中已经训过10轮的head
